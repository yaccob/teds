...
from teds_core.refs import _retrieve_with_config, NetworkConfiguration
...
class TestHttpRetrieve:
    ...
    def test_http_timeout_error(self):
        ...
        with patch('teds_core.refs.urlopen') as mock_urlopen:
            ...
            assert "failed to fetch http://example.com/schema.yaml" in str(exc_info.value)
    ...
    def test_http_successful_small_response(self):
        ...
        mock_resp = MagicMock()
        mock_resp.read.side_effect = [yaml_content.encode('utf-8'), b""]  # End with empty chunk
        ...
        with patch('teds_core.refs.urlopen') as mock_urlopen:
            ...
            mock_urlopen.assert_called_once_with("http://example.com/schema.yaml", timeout=config.timeout)
