Feature: HTTP API Local Server
  ...
  Scenario: Serve static files with proper MIME types
    Given I have files:
    ...
    And the HTTP API server is running
    When I request GET "/static/test.yaml"
    Then the response should have content-type "application/x-yaml"
    When I request GET "/static/test.json"
    Then the response should have content-type "application/json"
    When I request GET "/static/test.html"
    Then the response should have content-type "text/html"
    When I request GET "/static/test.js"
    Then the response should have content-type "application/javascript"

  Scenario: Static file serving with caching headers
    Given I have a schema file "cached.yaml"
    And the HTTP API server is running
    When I request GET "/static/cached.yaml"
    Then the response should have cache-control headers
    And the response headers should indicate development mode caching

  # ==========================================================================
  # Error Handling and Security
  # ==========================================================================

  Scenario: Handle CORS for local development
    Given the HTTP API server is running
    When I make a CORS preflight request to "/api/files"
    Then the response should include proper CORS headers
    And the response should allow common HTTP methods
    And the response should have status code 204
  ...
  Scenario: Handle server errors gracefully
    ...
    And the error details should be logged but not exposed

  Scenario: Handle malformed requests
    Given the HTTP API server is running
    When I send a request with malformed JSON to "/api/validate"
    Then the response should have status code 400
    And the response should contain "Invalid request format"
  ...
  Scenario: Server configuration via environment variables
    Given I set environment variable "TEDS_HTTP_PORT" to "9000"
    And I set environment variable "TEDS_HTTP_ROOT" to "/custom/root"
    When I start the HTTP API server
    Then the server should start on port 9000
    And the server should use "/custom/root" as root directory

  Scenario: Server configuration via config file
    Given I have a config file "teds_http.yaml" with content:
      """yaml
      server:
        port: 8888
      ...
      """
    When I start the HTTP API server with config file "teds_http.yaml"
    Then the server should use the configuration from the file
    And the server should start on port 8888
    And file watching should be enabled

  Scenario: Server status and health endpoints
    Given the HTTP API server is running
    When I request GET "/api/health"
    Then the response should have status code 200
    And the response should contain server status information
    When I request GET "/api/status"
    Then the response should contain runtime information
    And the response should include uptime and memory usage
